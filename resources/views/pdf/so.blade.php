...
<div class="header">
    ...
    <hr>
    <h3 style="margin-top: 10px;">SALES ORDER</h3>
</div>

<table class="info-table">
    <tr><td class="label">Nomor SO</td><td>: {{ $so->so_number }}</td><td class="label">Tanggal</td><td>: {{ $so->date->format('d M Y') }}</td></tr>
    <tr><td class="label">Customer</td><td>: {{ $so->customer?->name ?? '-' }}</td><td class="label">Status</td><td>: {{ $so->status }}</td></tr>
</table>

<table class="items-table">
    <thead>
        <tr><th>No</th><th>Barang</th><th class="right">Qty</th><th class="right">Harga</th><th class="right">Subtotal</th></tr>
    </thead>
    <tbody>
    ...
    <tfoot>
        <tr><th colspan="4" class="right">Total</th><th class="right">Rp {{ number_format($so->total_amount, 0, ',', '.') }}</th></tr>
    </tfoot>
</table>

<p><strong>Catatan:</strong> {{ $so->notes ?? '-' }}</p>
...
<div class="footer">
    <table class="footer-table">
        <tr>
            <td>
                <p>Dibuat oleh,</p>
                <div class="signature-line">{{ $so->creator?->name ?? '-' }}</div>
            </td>
            <td>
                <p>Disetujui oleh,</p>
                @if($company?->signature_image)
                    <img src="{{ public_path('storage/' . $company->signature_image) }}" style="max-height: 50px; margin-bottom: 5px;">
                @endif
                <div class="signature-line">{{ $company?->signature_name ?? '.........................' }}</div>
            </td>
        </tr>
    </table>
</div>
